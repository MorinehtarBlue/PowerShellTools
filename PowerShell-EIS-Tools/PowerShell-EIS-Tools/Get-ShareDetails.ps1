...
param (
	[Parameter(Mandatory=$true)][string]$serverName = "localhost",
	[string]$Path = "c:\temp")
 
$date = get-date
$datefile = get-date -uformat '%m-%d-%Y-%H%M%S'
$filename = $Path + "\" + $serverName + '.csv'
...
$strWMI = "\\" + $serverName + "\root\cimv2:win32_LogicalShareSecuritySetting.Name='" + $share.name + "'"
...
$shares = gwmi Win32_Share -computername $serverName -filter 'type=0'
